...
export const PixelGrid: React.FC = () => {
  const { canvas, zoom, setPixel } = useCanvasStore();
  ...
  const getPixel = (e: React.MouseEvent) => {
  ...
  return (
    <canvas ref={canvasRef} width={canvas.width * zoom} height={canvas.height * zoom}
      ...
      onMouseDown={e => { drawingRef.current = true; setPixel(...getPixel(e)); }}
      onMouseMove={e => { if (drawingRef.current) setPixel(...getPixel(e)); }}
